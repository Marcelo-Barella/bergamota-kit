...
`bergamota-kit` bundles 9 skills the author uses daily to drive Cursor agents through planning, research, debugging, and release work. Skills are loaded by Cursor on demand and invoked by the agent when the situation fits their trigger description.
...
Then in Cursor run Command Palette → "Developer: Reload Window" and check Settings → Skills for the 9 entries below.
...
| `subagent-build-plan` | Builds execution-ready subagent dispatch plans—Task waves, per-agent prompts, agent types, parallelism, and handoff contracts. Use when breaking work into parallel or sequential Task runs, designing multi-agent execution, or converting a goal into concrete subagent prompts before delegation. |
